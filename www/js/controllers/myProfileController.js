...
angular.module('controllers').controller('MyProfileController', function ($scope, $timeout, $ionicModal, $ionicPopup, $ionicSlideBoxDelegate, $cordovaCamera, $cordovaFileTransfer, $ionicActionSheet, GenericController, User, mainFactory) {
    ...
    $scope.getMyInfo = function () {
        mainFactory.me({ "e-posta": $scope.getUserFromLS().email }).then(successCallback, errorCallback);
    };
    ...
    $scope.saveProfileChanges = function () {
        if (!$scope.user.photos[0]) {
            $scope.showMessage("Bir profil fotoğrafı seçmelisin!", 2000);
            return;
        }
    ...
    function successUpdateUserInfo(response) {
        ...
        User.setUser(response.data.user);
        $scope.user = User.getUser();
        $scope.showMessage("Bir profil fotoğrafı seçmelisin!", 1500);
        $scope.editingProfile = false;
    ...
    $scope.showActionSheet = function() {
        $ionicActionSheet.show({
            titleText: 'Yüklü Profil Resimleri',
            buttons: [
                { text: '<i class="icon ion-camera"></i> Bir fotoğraf çek' },
    ...
    $scope.takePictureWithCamera = function() {
        ...
        $cordovaCamera.getPicture(options).then(function (imageURI) {
            ...
            console.log(err);
            $scope.hideMessage();
            $scope.showMessage('Fotoğraf seçerken hata!', 2500);
        });
    };
