...
angular.module('controllers').controller('MatchingController', function ($scope, $ionicSlideBoxDelegate, $ionicModal, $ionicPopup, GenericController, socket, User, mainFactory) {
    ...
    function init() {
        ...
        $scope.filters = {
            ...
            ageFrom: 18,
            ageTo: 35,
            gender: {'Kadın' : $scope.userProfile.gender == 'Erkek', 'Kadın': $scope.userProfile.gender == 'Kadın'},
    ...
    function makeUserFavoriteSuccess(response) {
        $scope.showMessage("Kullanıcı favorilere eklendi", 1000);
    }
    ...
    function removeUserFavoriteSuccess(response) {
        $scope.showMessage("Kullanıcı favorilerden kaldırıldı", 1000);
    }
    ...
    $scope.applyFilters = function () {
        var filters = {
            "İsim": $scope.userProfile.id,
            "E-posta": $scope.getUserFromLS().email,
            "Yaş": { ageFrom: $scope.filters.ageFrom, ageTo: $scope.filters.ageTo }
        };
        if ($scope.filters.interest) {
            filters.looking_to = $scope.filters.interest;
        }
        if (!$scope.filters.gender['Erkek'] || !$scope.filters.gender['Kadın']) {
            filters.gender = $scope.filters.gender['Erkek'] ? 'Kadın' : 'Kadın';
        }
