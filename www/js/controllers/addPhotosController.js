...
angular.module('controllers').controller('AddPhotosController', function ($scope, $ionicActionSheet, $cordovaCamera, $cordovaFileTransfer, GenericController, mainFactory, User) {
    ...
    $scope.showActionSheet = function() {
        $ionicActionSheet.show({
            titleText: 'Profil resmi yükleniyor',
            buttons: [
                { text: '<i class="icon ion-camera"></i> Fotoğraf çek' },
                { text: '<i class="icon ion-images"></i> Telefondan seç' }
            ],
            cancelText: 'İptal',
            cancel: function() {
    ...
    $scope.continueToMatching = function () {
        if ($scope.totalPics < 1) {
            $scope.showMessage("En az bir resim yüklemelisiniz!", 2000);
            return;
        }
        if (!$scope.pics[0]) {
            $scope.showMessage("Bir profil fotoğrafı seçmelisiniz!", 2000);
            return;
        }
        
        //update user pictures in the db
        $scope.showMessageWithIcon("Fotoğraf kaydediliyor...");
        ...
        mainFactory.updateNewUserPics({ user_id: User.getUser().id, amazon_pics: pics }).then(function (response) {
            ...
            $scope.hideMessage();
            $scope.showMessage("Birşeyler ters gitti!", 2000);
        });
    };
    ...
    $scope.takePictureWithCamera = function() {
        ...
        $cordovaCamera.getPicture(options).then(function (imageURI) {
            $scope.showMessage("Resim yükleniyor...");
            $scope.picData = imageURI;
            uploadToS3(imageURI);
        }, function (err) {
            console.log(err);
            $scope.hideMessage();
            $scope.showMessage('Fotoğraf çekerken hata oldu!', 2500);
        });
    };
    ...
    $scope.uploadImageToAmazon = function (data, imageURI, fileName) {
        ...
        $cordovaFileTransfer.upload(data.baseUrl, imageURI, Uoptions)
            .then(function (result) {
                ...
                $scope.hideMessage();
                $scope.showMessage("Resim yüklenirken hata oluştu!", 2000);
            }, function (progress) {
