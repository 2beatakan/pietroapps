...
angular.module('controllers').controller('LoginController', function ($scope, $q, $cordovaFacebook, $cordovaGeolocation, GenericController, User, mainFactory) {
    ...
    $scope.loginWithEmail = function () {
        if ($scope.user.email == undefined) {
            if (!$scope.validateEmail($scope.user.email)) {
                $scope.showMessage("Geçersiz e-posta adresi!", 2500);
                return;
            }
        }
        if (!$scope.user.email) {
        ...
        $scope.showMessageWithIcon("Lokasyon alınıyor...");
            ...
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    if ($scope.loginStatusFb === 'connected' || $scope.loginStatusFb === 'noFacebook') {
                        $scope.hideMessage();
                        $scope.showMessageWithIcon("Kimlik bilgileri doğrula...");
                        var credentials = {
                        ...
                        $scope.hideMessage();
                        $scope.showMessageWithIcon("Hesabını kaydediliyor");
    ...
    function authenticateError(response) {
        ...
        if (response.data) {
            ...
            $scope.showMessage("Birşeyler ters gitti!", 2500);
        }
    }
        ...
        facebookConnectPlugin.getLoginStatus(function (success) {
            if (success.status === 'connected') {
                ...
                $scope.getFacebookProfileInfo(success.authResponse).then(function (profileInfo) {
                    ...
                    $scope.showMessageWithIcon("Yer alınıyor...");
                    $scope.getCurrentLocation().then(successGetLocation, $scope.errorGetLocation);
                }, function (fail) {
    ...
    function errorCheckEmail(response) {
        $scope.showMessage(response.data ? response.data.error : "Servere bağlanamıyor", 3000);
    }
